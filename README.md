...
| API | transfer outbox replay | replay for `entity='transfer'` outbox rows is REAL (#55: source-namespace dedupe, lot-exact landing from `alloc_json`, shortfall restore, poisoned-row isolation, `updated_at` LWW watermark); REMAINING GAP: a peer offline for the ENTIRE flow applies the first pending row of a transfer and skips later transitions (existence-check pattern) — final-state convergence for that case needs a status-ahead delta rule | #55 (follow-up) |
