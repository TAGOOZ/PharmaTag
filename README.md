...
| API | transfer outbox replay | `entity='transfer'` `sync_log` rows ARE written atomically on every transition (G12) but `sync/replay_pending` still handles only `invoice`/`branch_stock` — offline-peer convergence for transfers needs a follow-up slice (idempotent replay keyed on `UNIQUE(source_branch_id, transfer_no)`); server-API flow itself is fully real since #32 | #32 (follow-up) |
