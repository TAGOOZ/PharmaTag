...
| API | branch settings | REAL (#59): `GET/PATCH /api/v1/branches/{id}/settings` (gated `branches.manage` floor 7) — `vat_inclusive_prices` (G06 retail-only), `treasury_enabled` (A19), `printer_defaults` per-purpose (F20.3 5 roles), branch legal fields (`pharname`/`adress`/`tax_id`), `PHARMATAG_TIMEZONE` read, G12 atomic audit+outbox, SQLite twin rev 036 + desktop bundle mirrored (`parity_check` green); `/settings` screen remains stub for #38 | #59 |
| API | transfer outbox replay | REAL (#55): versioned-apply keyed on `transfers.rev` (monotonic per-transition watermark, rev 030) — duplicates and out-of-order copies skip via `rev ≤ local`, missed transitions fold forward through the legal chain with BRANCH-FILTERED effects (source peer applies decrements/shortfall restore, target peer applies lot landings), all in one atomic txn; poisoned rows stay pending. `updated_at` LWW watermark is diagnostics-only | — |
