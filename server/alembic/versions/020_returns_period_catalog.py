"""core rev 020 — returns register catalog row (ticket #26, S3.4)

S3.4 adds the returns register to the `report_catalog` table.
Engine registers in app/reports/views.py; this row is inert data until then.
"""
from alembic import op
import sqlalchemy as sa

revision = "020_returns_period_catalog"
down_revision = "019_purchase_invoices_catalog"
branch_labels = None
depends_on = None

SEED_ROWS = [
    # code, category, title_ar, title_en, params, sort
    ("returns_period", "sales", "مرتجعات الفترة", "Period Returns",
     '["date_from", "date_to"]', 110),
]
